...
def tokenize(df):
    def fn(ex):
        ...
        mi = tokenizer(inputs, max_length=192, truncation=True, padding=False)
        mi["labels"] = tokenizer([str(t) for t in ex["translation"]], max_length=192, truncation=True, padding=False)["input_ids"]
        return mi
...
# ByT5-Base: smaller batch size, higher gradient accumulation to fit in GPU memory
