# ==============================================================================
# NOTEBOOK 3 of 4: Train NLLB-200  (~4-6 hours on Kaggle T4)
# ==============================================================================
...
# --- CELL 2: Preprocessing (identical to Notebooks 1 & 2) ---
...
def prepare_data():
    ...
    # Lexicon augmentation (NLLB also benefits from dictionary pairs)
...
# --- CELL 3: Train NLLB-200 with custom Akkadian token ---
...
def tokenize_nllb(df):
    def fn(ex):
        tokenizer.src_lang = AKKADIAN_TOKEN
        mi = tokenizer(ex["transliteration"], max_length=256, truncation=True, padding=False)
        tokenizer.src_lang = "eng_Latn"
        mi["labels"] = tokenizer(text_target=ex["translation"], max_length=512, truncation=True, padding=False)["input_ids"]
        tokenizer.src_lang = AKKADIAN_TOKEN
        return mi
...
train_ds, val_ds = tokenize_nllb(train_df.dropna()), tokenize_nllb(val_df.dropna())
...
trainer = Seq2SeqTrainer(
    ...
    args=Seq2SeqTrainingArguments(
        ...
        gradient_accumulation_steps=8, learning_rate=1e-4, warmup_steps=500, weight_decay=0.01,
...
print("Training NLLB-200 with akk_Latn token...")
trainer.train()
